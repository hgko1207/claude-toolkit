...
```
/autoplan 또는 /plan-eng-review 없이 바로 코딩하면 나중에 구조 뜯어야 함.
5분 투자 → 나중에 몇 시간 절약.
```
...
```
프로젝트마다 한 번 실행하면 이후 /ship, /land-and-deploy, /canary가 자동으로 연결됨.
새 프로젝트 시작하면 바로 실행하는 습관.
```

### 3. /setup-browser-cookies 인증 설정

```
인증이 필요한 페이지가 있는 앱은 /qa, /design-review 전에 항상 실행.
한 번 설정하면 세션 동안 유지됨.
```
...
```
큰 의존성 업데이트 후, 새 외부 API 연동 후, 배포 전 중요한 릴리즈 전에 실행.
보안 문제는 나중에 발견할수록 비용이 커짐.
```

### 5. /retro로 패턴 발견

```
매주 /retro 실행하면 어떤 버그가 반복되는지, 어디서 시간이 낭비되는지 패턴이 보임.
그 패턴을 CLAUDE.md에 기록하면 Claude가 반복 실수를 피할 수 있음.
```

### 6. /careful은 프로덕션 작업 기본값

```
프로덕션 DB, 라이브 서버, 공유 인프라 작업 시 항상 먼저 실행.
실수 한 번이 서비스 다운으로 이어질 수 있을 때.
```
