...
현재 브랜치와 base 브랜치의 diff를 분석해서 코드 리뷰를 수행한다.
일반적인 코드 품질 외에 **구조적 위험**에 집중한다.
...
현재 git diff를 자동으로 읽음. PR 번호나 브랜치를 지정할 수도 있음:
...
- `/ship` 전에 반드시 실행 — 배포 전 마지막 안전망
- "SQL 부분만 집중해서 봐줘"처럼 범위 지정 가능
- 수정까지 원하면 "수정도 해줘" 추가 요청
...
이후 Claude가 어떤 버그/오류인지 물어봄. 또는 바로 설명:
...
- 에러 메시지, 스택 트레이스, 재현 방법을 같이 제공하면 훨씬 빠름
- "증상만 숨기는 workaround 하지 마"라고 명시하면 근본 원인 수정에 집중
- 복잡한 버그는 `/investigate` → 원인 파악 → 직접 수정 or Claude 수정 선택
...
OpenAI Codex CLI를 사용해서 **3가지 모드**로 추가 관점을 제공한다.
Claude의 결과를 독립적으로 검증하거나, 다른 시각으로 코드를 테스트하는 데 유용.
...
- Challenge 모드: "내 코드가 얼마나 견고한가?" 확인에 특히 유용
- Claude + Codex 두 개가 동의하면 상당히 안전한 코드
- 의견이 다를 때가 제일 흥미롭고 배울 점이 많음
...
최근 변경된 코드를 검토하여 **재사용성, 품질, 효율성** 문제를 발견하고 수정한다.
...
- 기능 구현 완료 후 배포 전에 한 번 실행하면 좋음
- 너무 큰 리팩토링은 원하지 않으면 "사소한 것만" 명시
- `/review` (구조적 문제) + `/simplify` (코드 품질) 조합이 이상적
...
Claude가 위험한 명령어를 실행하기 전에 **경고를 표시**하고 사용자 확인을 요청한다.
...
이후 세션 전체에 적용됨. 각 위험 명령어마다 확인 요청.

### 꿀팁

- 프로덕션 디버깅 시 항상 먼저 실행
- 각 경고에서 재정의 가능 → 알고 하는 것과 실수로 하는 것을 구분
- `/guard` = `/careful` + `/freeze` 조합 (더 강력)
...
세션 동안 **지정한 디렉터리 외 파일 편집을 차단**한다.
Edit, Write 도구가 해당 경로 밖 파일에 접근하면 차단됨.
...
- `/freeze`와 `/careful`을 조합하려면 `/guard` 사용
...
`/careful` (위험 명령어 경고) + `/freeze` (디렉터리 편집 제한)를 **동시에 활성화**한다.
