...
Playwright 기반의 빠른 헤드리스 브라우저로 사이트를 직접 열어서 테스트한다.
gstack의 대부분 시각적 명령어(`/qa`, `/design-review`, `/canary`, `/benchmark`)가 내부적으로 이걸 사용한다.
...
- 직접 `/browse`를 쓰기보다 `/qa`, `/design-review` 같은 상위 명령어를 주로 사용
- 특정 시나리오만 테스트할 때 `/browse`로 직접 지시하면 유용
- 인증 필요 시 먼저 `/setup-browser-cookies` 실행
...
로컬 크롬 브라우저의 쿠키를 헤드리스 세션으로 가져온다.
로그인이 필요한 페이지를 테스트할 때 필수.
...
대화형 UI가 열리며 어떤 도메인의 쿠키를 가져올지 선택.
...
웹 앱을 체계적으로 QA 테스트하고 발견된 버그를 **자동으로 수정**한다.
각 수정을 원자적으로 커밋하고, 수정 후 재검증.
...
- 수정을 원하지 않으면 `/qa-only` 사용
- 인증 필요 페이지는 `/setup-browser-cookies` 먼저
- "결제 플로우만 테스트해줘"처럼 범위 지정 가능
- 발견된 버그가 많으면 우선순위를 물어봐도 됨
...
`/qa`와 동일하게 테스트하지만 **코드 수정은 절대 안 한다**.
버그 목록, 헬스 스코어, 스크린샷, 재현 방법이 포함된 구조화된 리포트 생성.
...
페이지 로드 타임, Core Web Vitals, 리소스 크기를 측정하고 **회귀 감지**를 한다.
기준선(baseline)을 수립하고, 이후 측정값과 비교해서 성능 저하를 감지.
...
를 CLAUDE.md에 저장한다. `/land-and-deploy`와 `/canary`가 이 정보를 사용함.
...
- "테스트 건너뛰어도 돼"처럼 특정 단계 생략 요청 가능
- CHANGELOG가 없는 프로젝트는 자동으로 생성
- `/review` 후 `/ship`이 이상적인 순서
...
`/ship`으로 만든 PR을 머지하고, CI/CD 완료를 기다리고, 프로덕션 헬스체크까지 한다.
...
`/setup-deploy`로 배포 설정이 되어 있어야 함.

#### 꿀팁

- 단독으로도 쓸 수 있지만 `/ship` → `/land-and-deploy` 순서가 표준
- CI가 느리면 기다리는 동안 다른 작업 가능 (백그라운드 대기)
...
배포 후 프로젝트 문서를 실제 출시된 내용과 동기화한다.
...
최근 diff를 읽고 모든 관련 문서를 자동으로 업데이트함.
