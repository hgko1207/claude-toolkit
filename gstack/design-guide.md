...
제품을 분석하고, 유사 제품을 리서치한 후, **완전한 디자인 시스템을 제안**한다.
결과물로 `DESIGN.md`를 생성해서 프로젝트의 디자인 소스 오브 트루스로 사용.
...
Claude가 제품의 성격, 타겟 유저, 브랜드 방향을 먼저 물어봄.
...
- 경쟁 제품이나 레퍼런스 디자인을 함께 언급하면 방향 잡기 쉬움
- 생성된 `DESIGN.md`를 `CLAUDE.md`에서 `@DESIGN.md`로 참조하면 이후 UI 작업에 자동 적용
- `/browse`로 레퍼런스 사이트를 함께 열어두면 더 정확한 결과
...
**실행 중인 웹사이트를 헤드리스 브라우저로 열어서** 시각적 문제를 발견하고 코드를 직접 수정한다.

발견 → 수정 → 재검증을 반복하며 각 수정을 원자적으로 커밋.
...
개발 서버가 실행 중이어야 함. URL을 자동으로 감지하거나:
```
...
- 수정 전후 스크린샷이 자동으로 저장됨 → diff로 비교 가능
- "이 페이지만 봐줘"처럼 범위 지정 가능: `/design-review http://localhost:3000/dashboard`
- 인증이 필요한 페이지는 먼저 `/setup-browser-cookies`로 쿠키 세팅
- `/plan-design-review`(플랜 검토)와 `/design-review`(라이브 감사)는 다른 명령어
...
작성된 플랜에서 **디자인 관련 항목을 0~10점으로 채점**하고, 10점이 되려면 뭐가 필요한지 설명한 후 플랜을 업데이트한다.
...
`/design-consultation`으로 생성한 `DESIGN.md`를 `CLAUDE.md`에 연결:
...
이후 Claude가 UI 코드를 작성할 때 자동으로 디자인 시스템을 참조한다.
