...
> CLAUDE.md, settings.json, 권한 설정 — 모든 프로젝트에서 첫 번째로 해야 할 것들입니다

---

## 1. CLAUDE.md — 세션의 두뇌

Claude Code가 시작될 때 **가장 먼저 읽는 파일**입니다. 이 파일이 Claude의 "작업 지시서"입니다.
...
`@` 문법으로 다른 파일을 참조하면 해당 파일 내용을 컨텍스트에 포함시킵니다.

### 유지 관리 원칙

- **200줄 이하 유지**: 200줄 이하 → 92% 이상 규칙 적용. 400줄 초과 → 71%로 급감합니다
- **자주 정리**: 코드처럼 관리하세요. 문제 발생하면 즉시 수정하세요
- **팀 기여**: git에 커밋해서 팀 전체가 함께 관리하세요
- **강조 표시**: 중요한 규칙은 `IMPORTANT:` 또는 `YOU MUST:` 사용하세요
- **자동 생성**: `/init` 명령으로 현재 프로젝트에 맞는 초안을 자동 생성할 수 있습니다
...
**deny → ask → allow** 순으로 평가됩니다 — 첫 번째 매칭이 적용됩니다
