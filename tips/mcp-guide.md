...
> Claude Code에 외부 도구, 데이터베이스, API 접근 권한을 부여하는 방법

---

## MCP란

MCP 서버는 Claude Code가 **외부 시스템과 연결**할 수 있게 해주는 플러그인이다.
GitHub, Slack, 데이터베이스, 브라우저 자동화 등을 Claude가 직접 사용할 수 있게 된다.

### 토큰 소비 주의사항

- MCP 서버가 많으면 도구 목록이 시스템 프롬프트에 포함되어 토큰을 소비
- Tool Search 기능(Sonnet 4+, Opus 4+에서 자동 활성화) 덕분에 서버 수에 관계없이 ~8,700 토큰으로 유지
- Tool Search 없이 50개 이상 도구 → 50K~100K 토큰 소비
- **결론**: 최신 모델에서는 MCP를 많이 설치해도 괜찮다. 단, 동시에 켜두는 것은 5~6개로 제한 (오래된 모델 사용 시)
...
이 설정을 `~/.claude/settings.json`에 추가하면 `.mcp.json`의 모든 서버가 자동으로 활성화된다.
...
1. **환경 변수로 시크릿 관리**: API 키를 .mcp.json에 직접 쓰지 말 것
2. **필요한 것만 켜두기**: 사용하지 않는 서버는 비활성화 (토큰 절약)
3. **.mcp.json 보안 검토**: git에 커밋하기 전 민감 정보 확인
4. **권한 최소화**: 읽기만 필요하면 쓰기 권한 차단
