...
> 특정 시점에 자동으로 실행되는 커맨드입니다. "Claude가 파일을 수정할 때마다 자동으로 포맷터를 실행해줘" 같은 것들입니다.

---

## 훅이란

Claude Code의 **훅(Hook)**은 특정 이벤트가 발생했을 때 자동으로 실행되는 쉘 명령어입니다.
Claude한테 부탁하는 게 아니라, Claude가 특정 동작을 할 때 **자동으로 강제 실행**된다는 게 핵심입니다.
...
Claude가 "완료했습니다" 하고 끝내려는데 테스트가 실패하면? 이 Hook을 걸면 테스트 통과 전까지 끝낼 수 없습니다.
...
exit 2 = Claude에게 "끝내지 마, 이거 먼저 해결해" 신호입니다. 솔로 개발에서 QA 자동화의 핵심입니다.
...
프롬프트에 특정 키워드가 포함되면 관련 스킬을 자동으로 주입합니다. 매번 `/skill-name` 수동 호출할 필요가 없습니다.
...
여러 도메인을 넘나드는 풀스택 개발에서 특히 효과적입니다.
