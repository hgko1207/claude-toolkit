...
> 재사용 가능한 워크플로우(Skills)와 전문 AI 팀원(Sub-agents)을 만드는 법
...
스킬은 `/deploy`, `/fix-issue`, `/review` 같은 **슬래시 명령어**로 호출할 수 있는 재사용 가능한 워크플로우다.
SKILL.md 파일 하나로 만들 수 있으며, 지원 파일(템플릿, 예시, 스크립트)을 함께 포함할 수 있다.
...
`!`` 문법으로 실행한 명령어 결과가 Claude가 보기 전에 삽입된다.
...
서브에이전트는 **독립된 컨텍스트 창**에서 실행되는 전문 AI 어시스턴트다.
각 에이전트는 고유한 시스템 프롬프트, 허용된 도구 목록, 독립적인 권한을 가진다.

핵심 차이:
- **스킬**: 현재 대화의 컨텍스트 안에서 실행
- **서브에이전트**: 완전히 분리된 컨텍스트 창에서 실행 → 메인 대화 컨텍스트 보호
...
| `background` | `true`면 항상 백그라운드 작업으로 실행 |
| `isolation` | `worktree`면 격리된 git worktree에서 실행 |
...
`MEMORY.md` 파일의 처음 200줄이 에이전트 컨텍스트에 자동 주입된다.
