...
코딩부터 시작하면 잘못된 방향으로 달려가 토큰만 낭비합니다.
Plan 모드는 **읽기 전용**으로 동작해서 파일 수정이나 명령 실행 없이 분석만 합니다.

**효과**: 플랜 단계에서 실행 단계를 분리하면 토큰 40~60% 절약됩니다.
...
`Ctrl+G` → 플랜을 텍스트 에디터에서 직접 편집 가능합니다
...
Claude의 200K 토큰 컨텍스트 창에는 전체 대화가 들어갑니다.
대화가 길어질수록 앞 내용이 흐릿해지고 성능이 저하됩니다.
**컨텍스트 관리가 Claude Code 품질의 1번 요인입니다.**
...
- Claude가 모든 액션마다 자동으로 체크포인트를 생성합니다
- `Esc+Esc` 또는 `/rewind` → 복원 메뉴
- 대화만, 코드만, 또는 둘 다 복원 가능합니다
- 세션 간에도 유지됩니다
...
Git worktree는 **같은 저장소에서 여러 작업 디렉터리를 동시에** 만드는 기능입니다.
각 디렉터리는 독립적인 브랜치와 파일을 가집니다.
Claude Code에서는 각 디렉터리에서 별도 Claude 세션을 실행하면 **병렬 기능 개발이 가능**합니다.
...
워크트리는 `.claude/worktrees/<name>`에 생성됩니다.
`.gitignore`에 `.claude/worktrees/` 추가가 필수입니다.
...
- **변경/커밋 있음**: Claude가 유지 또는 삭제 여부를 확인합니다
...
`isolation: worktree`를 설정한 에이전트는 자동으로 격리된 워크트리에서 실행됩니다.
...
각 Claude 인스턴스는 독립적인 브랜치와 파일을 가지므로 충돌이 없습니다.

**주의사항:**
- 각 체크포인트에서 main을 merge해서 드리프트를 방지하세요
...
2. **30~45분 집중 세션**: 긴 방황 세션보다 짧고 집중된 세션이 효과적입니다
3. **각 기능마다 새 브랜치**: 워크트리 또는 세션 격리
4. **검증 인프라에 투자**: 테스트, 린터, 타입체커 — 이게 ROI 배수를 결정합니다
